...
import { Fp as sensitive } from "./auth-profiles-DZ4zIQH4.js";
...
const DISCORD_DEFAULT_INBOUND_WORKER_TIMEOUT_MS = 18e5;
...
const FIELD_HELP = {
	...
	"browser.relayBindHost": "Bind IP address for the Chrome extension relay listener. Leave unset for loopback-only access, or set an explicit non-loopback IP such as 0.0.0.0 only when the relay must be reachable across network namespaces (for example WSL2) and the surrounding network is already trusted.",
	...
	"browser.profiles.*.driver": "Per-profile browser driver mode: \"openclaw\" (or legacy \"clawd\") or \"extension\" depending on connection/runtime strategy. Use the driver that matches your browser control stack to avoid protocol mismatches.",
	...
	"tools.web.search.provider": "Search provider (\"brave\", \"gemini\", \"grok\", \"kimi\", or \"perplexity\"). Auto-detected from available API keys if omitted.",
...
function restoreRedactedValuesWithLookup(incoming, original, lookup, prefix, hints) {
	...
	for (const [key, value] of Object.entries(incoming)) {
		...
		for (const candidate of [path, wildcardPath]) if (lookup.has(candidate)) {
			matched = true;
			if (value === "__OPENCLAW_REDACTED__") result[key] = restoreOriginalValueOrThrow({
				key,
				path: candidate,
				original: orig
			});
		...
		if (!matched) {
			if (!isExplicitlyNonSensitivePath(hints, [path, wildcardPath]) && isSensitivePath(path) && value === "__OPENCLAW_REDACTED__") result[key] = restoreOriginalValueOrThrow({
				key,
				path,
				original: orig
			});
...
function restoreRedactedValuesGuessing(incoming, original, prefix, hints) {
	...
	for (const [key, value] of Object.entries(incoming)) {
		const path = prefix ? `${prefix}.${key}` : key;
		if (!isExplicitlyNonSensitivePath(hints, [path, prefix ? `${prefix}.*` : "*"]) && isSensitivePath(path) && value === "__OPENCLAW_REDACTED__") result[key] = restoreOriginalValueOrThrow({
			key,
			path,
			original: orig
		});
