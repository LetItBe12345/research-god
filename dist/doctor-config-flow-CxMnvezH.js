import { f as resolveLegacyStateDirs, g as resolveStateDir, m as resolveOAuthDir, p as resolveNewStateDir, t as CONFIG_PATH } from "./paths-dQ_clcF4.js";
import { $ as resolveCommandSecretRefsViaGateway, Dp as readConfigFileSnapshot, Fn as listRouteBindings, Ml as saveSessionStore, Np as OpenClawSchema, Sp as migrateLegacyConfig, Y as getChannelsCommandSecretTargetIds, dm as resolveDiscordPreviewStreamMode, fm as resolveSlackNativeStreaming, im as resolveCommandResolutionFromArgv, lm as formatSlackStreamModeMigrationMessage, lu as canonicalizeMainSessionAlias, mc as resolveMergedSafeBinProfileFixtures, mm as resolveTelegramPreviewStreamMode, nm as isTrustedSafeBinPath, pc as listInterpreterLikeSafeBins, pm as resolveSlackStreamingMode, rm as normalizeTrustedSafeBinDirs, tm as getTrustedSafeBinDirs, um as formatSlackStreamingBooleanMigrationMessage, wt as applyPluginAutoEnable, xp as parseToolsBySenderTypedKey } from "./auth-profiles-DZ4zIQH4.js";
import { t as formatCliCommand } from "./command-format-CO3N-oGG.js";
import { f as resolveDefaultAgentId } from "./agent-scope-lHks6gb9.js";
import { _ as normalizeOptionalAccountId, g as normalizeAccountId, h as DEFAULT_ACCOUNT_ID, n as DEFAULT_MAIN_KEY, r as buildAgentMainSessionKey, s as normalizeAgentId } from "./session-key-9lbhEIb9.js";
import { m as resolveHomeDir, u as isRecord } from "./utils-D0Yqqtk-.js";
import { i as coerceSecretRef, l as normalizeSecretInputString } from "./types.secrets-BrzD-dBG.js";
import { L as normalizeChatChannelId } from "./skills-NeRNNbEl.js";
import { n as formatConfigIssueLines } from "./issue-format-C8GXLHk1.js";
import { a as resolveChannelAllowFromPath, r as readChannelAllowFromStore } from "./pairing-store-CkFJMwFX.js";
import { t as shouldMoveSingleAccountChannelKey } from "./setup-helpers-ClwugVS0.js";
import { a as isMattermostMutableAllowEntry, c as collectProviderDangerousNameMatchingScopes, i as isMSTeamsMutableAllowEntry, n as isGoogleChatMutableAllowEntry, o as isSlackMutableAllowEntry, r as isIrcMutableAllowEntry, s as isZalouserMutableGroupEntry, t as isDiscordMutableAllowEntry } from "./mutable-allowlist-detectors-B0bD0YJN.js";
import { t as isWithinDir } from "./path-safety-C7XgwZ9I.js";
import { t as note } from "./note-QL6AfaP7.js";
...
function listTelegramAccountIds$1(_cfg) {
	return [];
}
...
async function detectLegacyStateMigrations(params) {
	...
	const telegramPairingAllowFromPlans = fileExists(legacyTelegramAllowFromPath) ? Array.from(new Set(listTelegramAccountIds$1(params.cfg).map((accountId) => resolveChannelAllowFromPath("telegram", env, accountId)))).filter((targetPath) => !fileExists(targetPath)).map((targetPath) => ({
...
function inspectTelegramAccount(params) {
	if (resolveTelegramAccount(params).tokenSource === "config") return {
		enabled: true,
		tokenStatus: "missing"
	};
	...
	if (coerceSecretRef((normalizedAccountId === "default" ? telegram : asObjectRecord(accounts?.[normalizedAccountId]))?.botToken ?? telegram?.botToken)) return {
		enabled: true,
...
function listTelegramAccountIds(cfg) {
	...
	if (telegram.botToken !== void 0) ids.add(DEFAULT_ACCOUNT_ID);
	const accounts = asObjectRecord(telegram.accounts);
	if (accounts) for (const key of Object.keys(accounts)) ids.add(normalizeAccountId(key));
	return [...ids];
}
function resolveTelegramAccount(params) {
	...
	const accounts = asObjectRecord(telegram?.accounts);
	const normalizedAccountId = normalizeAccountId(params.accountId);
	const token = normalizeSecretInputString((normalizedAccountId === "default" ? telegram : asObjectRecord(accounts?.[normalizedAccountId]))?.botToken) ?? normalizeSecretInputString(telegram?.botToken) ?? "";
	return token ? {
		tokenSource: "config",
		token
	} : {
...
function normalizeTelegramAllowFromEntry(entry) {
	return String(entry ?? "").trim().replace(/^tg:/i, "").trim();
}
...
async function fetchTelegramChatId(params) {
	...
	if (!payload?.ok || payload.result?.id == null) return null;
