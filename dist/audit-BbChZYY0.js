import { g as resolveStateDir, m as resolveOAuthDir, o as resolveConfigPath, t as CONFIG_PATH } from "./paths-dQ_clcF4.js";
import { $d as formatPermissionDetail, Cp as createConfigIO, En as hasResolvedCredentialValue, Hp as isDangerousNetworkMode, Lu as execDockerRaw, Pd as resolveBrowserControlAuth, Qf as resolveToolProfilePolicy, Ru as getBlockedBindReason, Tn as hasConfiguredUnavailableCredentialStatus, Up as normalizeNetworkMode, Yf as resolveSandboxToolPolicyForAgent, _n as listAgentWorkspaceDirs, bn as inspectReadOnlyChannelAccount, br as isToolAllowedByPolicies, df as resolveGatewayAuth, dl as resolveNativeCommandsEnabled, ef as formatPermissionRemediation, fl as resolveNativeSkillsEnabled, gu as listChannelPlugins, hd as resolveProfile, hm as INCLUDE_KEY, ka as buildGatewayConnectionDetails, ku as normalizeMessageChannel, mc as resolveMergedSafeBinProfileFixtures, md as resolveBrowserConfig, nf as safeStat, op as SANDBOX_BROWSER_SECURITY_HASH_EPOCH, pc as listInterpreterLikeSafeBins, qf as resolveSandboxConfigForAgent, rm as normalizeTrustedSafeBinDirs, tf as inspectPathPermissions, wr as pickSandboxToolPolicy, yu as resolveChannelDefaultAccountId } from "./auth-profiles-DZ4zIQH4.js";
import { t as formatCliCommand } from "./command-format-CO3N-oGG.js";
import { N as resolveAgentModelFallbackValues, P as resolveAgentModelPrimaryValue, f as resolveDefaultAgentId, r as listAgentIds } from "./agent-scope-lHks6gb9.js";
import { T as parseAgentSessionKey, s as normalizeAgentId } from "./session-key-9lbhEIb9.js";
...
import { E as normalizePluginsConfig, i as loadWorkspaceSkillEntries } from "./skills-NeRNNbEl.js";
import { m as MANIFEST_KEY } from "./frontmatter-BzGWVIyf.js";
import { n as isPathInside, t as extensionUsesSkippedScannerPath } from "./scan-paths-Fe1oJGGa.js";
import { n as DEFAULT_GATEWAY_HTTP_TOOL_DENY } from "./dangerous-tools-CMvz-16P.js";
import { r as probeGateway, t as resolveGatewayProbeAuthSafe } from "./probe-auth-BBmbxTvj.js";
import { r as readChannelAllowFromStore } from "./pairing-store-CkFJMwFX.js";
import { r as resolveNodeCommandAllowlist, t as DEFAULT_DANGEROUS_NODE_COMMANDS } from "./node-command-policy-Xa3CnNVq.js";
import { l as isDangerousNameMatchingEnabled, s as isZalouserMutableGroupEntry, t as isDiscordMutableAllowEntry } from "./mutable-allowlist-detectors-B0bD0YJN.js";
import { t as resolveDmAllowState } from "./dm-policy-shared-DsAESt5G.js";
import { t as inferParamBFromIdOrName } from "./model-param-b-Ckv3SH6X.js";
...
function normalizeTelegramAllowFromEntry(entry) {
	return String(entry ?? "").trim().replace(/^@+/, "@");
}
function isNumericTelegramUserId(value) {
...
async function collectIncludePathsRecursive(params) {
	...
	const walk = async (basePath, parsed, depth) => {
		...
		for (const raw of listDirectIncludes(parsed)) {
			...
			const rawText = await fs$2.readFile(resolved, "utf-8").catch(() => null);
			if (!rawText) continue;
			const nestedParsed = (() => {
				try {
...
async function readPluginManifestExtensions(pluginPath) {
	...
	const raw = await fs$1.readFile(manifestPath, "utf-8").catch(() => "");
	if (!raw.trim()) return [];
...
async function listInstalledPluginDirs(params) {
	...
	return {
		extensionsDir,
		pluginDirs: (await fs$1.readdir(extensionsDir, { withFileTypes: true }).catch((err) => {
...
async function listWorkspaceSkillMarkdownFiles(workspaceDir) {
	...
	while (queue.length > 0 && skillFiles.length < MAX_WORKSPACE_SKILL_SCAN_FILES_PER_WORKSPACE) {
		...
		const dirRealPath = await fs$1.realpath(dir).catch(() => path.resolve(dir));
...
async function collectWorkspaceSkillSymlinkEscapeFindings(params) {
	...
	for (const workspaceDir of workspaceDirs) {
		...
		const workspaceRealPath = await fs$1.realpath(workspacePath).catch(() => workspacePath);
		...
		for (const skillFilePath of skillFilePaths) {
			...
			const skillRealPath = await fs$1.realpath(canonicalSkillPath).catch(() => null);
...
//#endregion
export { resolveHookTargetAgentId as _, getHookAgentPolicyError as a, resolveFunctionModuleExport as b, normalizeAgentPayload as c, normalizeWakePayload as d, readJsonBody as f, resolveHookSessionKey as g, resolveHookIdempotencyKey as h, extractHookToken as i, normalizeHookDispatchSessionKey as l, resolveHookDeliver as m, collectEnabledInsecureOrDangerousFlags as n, getHookChannelError as o, resolveHookChannel as p, collectIncludePathsRecursive as r, isHookAgentAllowed as s, runSecurityAudit as t, normalizeHookHeaders as u, resolveHooksConfig as v, applyHookMappings as y };
