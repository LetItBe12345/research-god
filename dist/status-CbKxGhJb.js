import { Dn as projectSafeChannelAccountSnapshotFields, bn as inspectReadOnlyChannelAccount } from "./auth-profiles-DZ4zIQH4.js";
...
function inspectChannelAccount(params) {
	return params.plugin.config.inspectAccount?.(params.cfg, params.accountId) ?? inspectReadOnlyChannelAccount({
...
async function buildChannelAccountSnapshot(params) {
	const account = inspectChannelAccount(params) ?? params.plugin.config.resolveAccount(params.cfg, params.accountId);
	return await buildSnapshotFromAccount({
		...params,
		account
	});
