import { i as resolveProviderWizardOptions } from "./provider-wizard-Di6BkcUS.js";
import { n as AUTH_CHOICE_LEGACY_ALIASES_FOR_CLI, t as ONBOARD_PROVIDER_AUTH_FLAGS } from "./onboard-provider-auth-flags-BP8P5KzB.js";
...
function formatAuthChoiceChoicesForCli(params) {
	...
	const values = [...BASE_AUTH_CHOICE_OPTIONS.map((opt) => opt.value), ...resolveDynamicProviderCliChoices(params)];
...
function buildAuthChoiceGroups(params) {
	...
	for (const option of resolveProviderWizardOptions({
		...
		const nextOption = optionByValue.get(option.value) ?? {
			value: option.value,
			label: option.label,
			hint: option.hint
		};
		if (existing) {
			if (!existing.options.some((candidate) => candidate.value === nextOption.value)) existing.options.push(nextOption);
			continue;
		}
		if (staticGroupIds.has(option.groupId)) continue;
		groups.push({
		...
		staticGroupIds.add(option.groupId);
	}
	return {
		groups,
