import { Hu as readJsonFile, Ps as normalizeDeviceAuthScopes, Uu as writeJsonAtomic, Vu as createAsyncLock, fn as generatePairingToken, gn as resolvePairingPaths, hn as pruneExpiredPending, mn as rejectPendingPairingRequest, pn as verifyPairingToken } from "./auth-profiles-DZ4zIQH4.js";
...
async function rotateDeviceToken(params) {
	return await withLock(async () => {
		...
		if (!scopesWithinApprovedDeviceBaseline({
			role,
			scopes: requestedScopes,
			approvedScopes: resolveApprovedDeviceScopeBaseline(device)
		})) return null;
		const now = Date.now();
		const next = buildDeviceAuthToken({
			role,
